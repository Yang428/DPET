...
class DPETNet(nn.Module):
    def __init__(self, segm_input_dim=(128,256), segm_inter_dim=(256,256), segm_dim=(64, 64), mixer_channels=2, topk_pos=3, topk_neg=3):
        ...
        self.fusion = fusion_layer(d_model=64, dim_hider=256, nhead=8, dropout=0.1) ## attention based fusion net
   
        self.cor_conv0 = conv(segm_input_dim[3], segm_dim[0], kernel_size=1, padding=0) ## 1x1,3x3 conv for correlation net
        self.cor_conv1 = conv_no_relu(segm_dim[0], segm_dim[1])
        ...
        self.mixer0 = conv(mixer_channels, segm_inter_dim[2])   ## DPE-Net
    ...
    def forward(self, feat_test, feat_train, mask_train, test_dist=None, feat_ups=None, up_masks=None, segm_update_flag=False):
        ...
        out = self.mixer1(self.mixer0(segm_layers)) ## position features
      
        ## attention-based position-aware features
        f_att = self.fusion(self.att_conv1(self.att_conv0(feat_train[3])), self.att_conv1(self.att_conv0(feat_test[3])), out.sigmoid()) 

        out = self.s3_1(self.s3_0(F.interpolate(self.att_conv3(self.att_conv2(f_att)) + out, scale_factor=2, mode='bilinear', align_corners=False)))
