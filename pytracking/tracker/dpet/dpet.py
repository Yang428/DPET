...
class DPET(BaseTracker):
    ...
    def track(self, image):
        ...
        if uncert_score < self.params.tracking_uncertainty_thr:
            ...
            ''' for others          
            if (hard_negative or self.segm_hard_neg):
                if (self.frame_num-self.segm_update_num)>(self.params.segm_update_skip*2-1):  
                    # if there is hard negative in background, update 10 skipping
                    self.update_segm_sample(image, new_pos, self.target_sz)
                    self.segm_update_num = self.frame_num
            '''  
    ...
    def init_segmentation(self, image, bb, init_mask=None):
        ...
        if init_mask is not None:
            ...
            if hasattr(self, 'gt_poly'):
                ...
                cv2.line(im1,pp1,pp2,(0,255,0),2)
        ...
        segm_net, _ = load_network(self.params.segm_net_path, backbone_pretrained=False,
                                   constructor_module='ltr.models.dpet.dpet',
                                   constructor_fun_name='dpet_resnet50')
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:  
            ...
            if not self.params.segm_scale_estimation or pixels_ratio < self.params.segm_pixels_ratio:
                ...
                if self.frame_num==930:#True:
                    ...
                    patch1 = copy.copy(image)
                    cv2.line(patch1,p1,p2,(255,0,0),2)
                    cv2.line(patch1,p2,p3,(255,0,0),2)
                    cv2.line(patch1,p3,p4,(255,0,0),2)
                    cv2.line(patch1,p4,p1,(255,0,0),2)
                    #patch1, f1_ = prutils.sample_target(im1, np.array(bb), self.params.segm_search_area_factor,output_sz=self.params.segm_output_sz)
                    plt.figure(figsize=(10,10))
                    #plt.imshow(patch1)
                    #plt.figure(figsize=(10,10))
                    #plt.imshow(mask)
                    patch1[:,:,0] = patch1[:,:,0]*(1-final_mask) + patch1[:,:,0]*final_mask*0.5+final_mask*100
                    patch1[:,:,1] = patch1[:,:,1]*(1-final_mask) + patch1[:,:,1]*final_mask*0.5+final_mask*100
                    patch1[:,:,2] = patch1[:,:,2]*(1-final_mask) + patch1[:,:,2]*final_mask*0.5
                    patch1 = drawContours(patch1, [contour], -1, [0,255,0], thickness=1)
                    plt.imshow(patch1)
                    plt.xticks([])
                    plt.yticks([])
